...
namespace B3
{
    class Box3DScene final : public AzPhysics::Scene
    {
    public:
        AZ_CLASS_ALLOCATOR_DECL;
        AZ_RTTI(Box3DScene, "{3135EEC0-A7D2-465B-BE02-2B7CFD137872}", AzPhysics::Scene);

        explicit Box3DScene(const AzPhysics::SceneConfiguration& config, const AzPhysics::SceneHandle& sceneHandle);
        ~Box3DScene();
    ...
    private:
        ...
        public:
            ...
            void ApplyParallel(const AZStd::function<void(AzPhysics::SimulatedBodyIndex)>& applyFunction, b3WorldId worldId);
        ...
        AzPhysics::CollisionEventList m_queuedCollisionEvents; //!< Holds all the collision events the happened until the next simulation;
        AzPhysics::TriggerEventList m_queuedTriggerEvents; //!< Holds all the trigger events the happened until the next simulation;
