...
namespace B3
{
    ...
    {
    public:
        AZ_EBUS_BEHAVIOR_BINDER(BehaviorRigidBodyNotificationBusHandler, "{66CD44C6-9FBB-4FFD-A4B0-3A0D4FDA2050}", AZ::SystemAllocator,
            OnPhysicsEnabled,
            OnPhysicsDisabled
        );
    ...
    void RigidBodyComponent::Reflect(AZ::ReflectContext* context)
    {
        ...
        if (serializeContext)
        {
            serializeContext->Class<RigidBodyComponent, AZ::Component>()
                ...
                ->Field("Box3DSpecificConfiguration", &RigidBodyComponent::m_box3DSpecificConfiguration)
                ;
        }
    ...
    RigidBodyComponent::RigidBodyComponent(
        ...
        const RigidBodyConfiguration& box3DSpecificConfig,
        AzPhysics::SceneHandle sceneHandle)
        : m_configuration(baseConfig)
        , m_box3DSpecificConfiguration(box3DSpecificConfig)
        , m_attachedSceneHandle(sceneHandle)
    {
        InitPhysicsTickHandler();
    }
    ...
    void RigidBodyComponent::Activate()
    {
        ...
        {
            AZ_Warning("RigidBodyComponent", false, "It is not valid to have a Box3D Dynamic Rigid Body Component "
    ...
    void RigidBodyComponent::OnEntityActivated([[maybe_unused]] const AZ::EntityId& entityId)
    {
        ...
        // Add to world
        EnablePhysics(); // TODO: this can and should be done at creation with Box3D as this is expensive 
    }
    ...
    void RigidBodyComponent::CreateRigidBody()
    {
        ...
        if (m_cachedSceneInterface != nullptr)
        {
            m_configuration.m_startSimulationEnabled = false; //enable physics will enable this when called. // TODO: make this configurable
            m_rigidBodyHandle = m_cachedSceneInterface->AddSimulatedBody(m_attachedSceneHandle, &m_configuration);
            ApplyBox3DSpecificConfiguration();

            // Listen to the Box3D system for events concerning this entity.
    ...
    void RigidBodyComponent::ApplyBox3DSpecificConfiguration()
    {
        if (auto* body = GetRigidBody())
        {
            if (auto* bodyId = static_cast<b3BodyId*>(body->GetNativePointer()))
            {
                AZ_UNUSED(bodyId);
                // const AZ::u8 solverPositionIterations =
                //     AZ::GetClamp<AZ::u8>(m_box3DSpecificConfiguration.m_solverPositionIterations, 1, 255);
                // const AZ::u8 solverVelocityIterations =
                //     AZ::GetClamp<AZ::u8>(m_box3DSpecificConfiguration.m_solverVelocityIterations, 1, 255);
                // bodyId->setSolverIterationCounts(solverPositionIterations, solverVelocityIterations);
            }
        }
    }
