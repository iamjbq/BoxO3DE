...
namespace B3
{
    ...
    void BaseColliderComponent::SetShapeConfigurationList(const AzPhysics::ShapeColliderPairList& shapeConfigList)
    {
        ...
        {
            AZ_Warning("Box3D", false, "Trying to call SetShapeConfigurationList for entity \"%s\" while entity is active.",
    ...
    {
        AZ_Error("Box3D", !m_shapes.empty(), "Tried to call IsTrigger before any shapes were initialized for entity %s.",
            GetEntity()->GetName().c_str());
    ...
    bool BaseColliderComponent::InitShapes()
    {
        ...
        else
        {
            ...
            for (const auto& shapeConfigPair : m_shapeConfigList)
            {
                ...
                {
                    AZ_Error("Box3D", false, "Unable to create a physics shape because shape configuration is null. Entity: %s",
                        GetEntity()->GetName().c_str());
                    return false;
                }
                ...
                if (!shape)
                {
                    AZ_Error("Box3D", false, "Failed to create a Box3D shape. Entity: %s", GetEntity()->GetName().c_str());
                    return false;
                }
