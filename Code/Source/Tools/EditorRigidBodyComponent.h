...
namespace B3
{
    ...
    {
        AZ_CLASS_ALLOCATOR(B3::EditorRigidBodyConfiguration, AZ::SystemAllocator);
        AZ_RTTI(B3::EditorRigidBodyConfiguration, "{1EF7B5EE-9F8D-49B4-A8B8-308BEF820A35}", AzPhysics::RigidBodyConfiguration);
    ...
    //! Class for in-editor Box3D Dynamic Rigid Body Component.
    ...
    {
    public:
        AZ_EDITOR_COMPONENT(EditorRigidBodyComponent, "{D6FF85F5-7389-47D8-B654-FB1D9013820D}", AzToolsFramework::Components::EditorComponentBase);
        static void Reflect(AZ::ReflectContext* context);

        EditorRigidBodyComponent();
        explicit EditorRigidBodyComponent(const EditorRigidBodyConfiguration& configuration);
        EditorRigidBodyComponent(const EditorRigidBodyConfiguration& configuration, const RigidBodyConfiguration& box3DSpecificConfiguration);
    ...
    private:
        ...
        RigidBodyConfiguration
            m_box3DSpecificConfig; //!< Properties specific to Box3D which might not have exact equivalents in other physics engines.
