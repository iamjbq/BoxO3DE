...
namespace B3
{
    namespace DebugDraw
    {
        ...
        bool IsGlobalColliderDebugCheck(Debug::DebugDisplayData::GlobalCollisionDebugState requiredState)
        {
            ...
            {
                return box3DDebug->GetDebugDisplayData().m_globalCollisionDebugDraw == requiredState;
            }
            return false;
        }
        ...
        Collider::Collider()
            : m_debugDisplayDataChangedEvent(
                  [this]([[maybe_unused]] const B3::Debug::DebugDisplayData& data)
                  {
        ...
        void Collider::BuildMeshes(const Physics::ShapeConfiguration& shapeConfig, AZ::u32 geomIndex) const
        {
            ...
            case Physics::ShapeType::CookedMesh:
            {
                ...
                const b3HullData* constMeshData = static_cast<const b3HullData*>(cookedMeshConfig.GetCachedNativeMesh());
                
                // Specifically removing the const from the meshData pointer because the physx APIs expect this pointer to be non-const.
                b3HullData* meshData = const_cast<b3HullData*>(constMeshData);
            ...
            default:
            {
                AZ_Error("Box3D", false, "DebugDraw::Collider::BuildMeshes: Unsupported ShapeType %d. Entity %s, ID: %llu",
        ...
        //     {
        //         AZ::Vector3 vert = Box3DMathConvert(vertices[vertIndex]);
        ...
        void Collider::BuildConvexMesh(b3HullData* hullData, AZ::u32 geomIndex) const
        {
            GeometryData& geom = m_geometry[geomIndex];
            ...
            , const ElementDebugInfo& elementDebugInfo) const
        {
            ...
            if (auto* box3DDebug = AZ::Interface<Debug::Box3DDebugInterface>::Get())
            {
                debugDrawColorMode = box3DDebug->GetDebugDisplayData().m_globalCollisionDebugDrawColorMode;
            }
            ...
            AzFramework::DebugDisplayRequests& debugDisplay)
        {
            ...
            auto* box3DDebug = AZ::Interface<Debug::Box3DDebugInterface>::Get();
            if (box3DDebug == nullptr)
            {
                return;
            }
            const B3::Debug::DebugDisplayData& displayData = box3DDebug->GetDebugDisplayData();

            const B3::Debug::ColliderProximityVisualization& proximityVisualization = displayData.m_colliderProximityVisualization;
            ...
            const B3::Debug::DebugDisplayData::GlobalCollisionDebugState& globalCollisionDebugDraw = displayData.m_globalCollisionDebugDraw;
            if (globalCollisionDebugDraw != B3::Debug::DebugDisplayData::GlobalCollisionDebugState::AlwaysOff)
            {
                if (globalCollisionDebugDraw == B3::Debug::DebugDisplayData::GlobalCollisionDebugState::AlwaysOn
                    || m_locallyEnabled
        ...
        void Collider::OnSelected()
        {
            ...
            if (auto* box3DDebug = AZ::Interface<Debug::Box3DDebugInterface>::Get())
            {
                box3DDebug->RegisterDebugDisplayDataChangedEvent(m_debugDisplayDataChangedEvent);
            }
        }
