...
namespace B3
{
        ...
        AssetBuilderSDK::CreateJobsResponse& response) const
    {
        for (const AssetBuilderSDK::PlatformInfo& platformInfo : request.m_enabledPlatforms)
        {
            ...
            jobDescriptor.m_jobKey = "Box3D Material Asset";
        ...
        AssetBuilderSDK::ProcessJobResponse& response) const
    {
        ...
        {
            AZ_Error("EditorMaterialAssetBuilder", false, "Failed to load Box3D EditorMaterialAsset: %s", request.m_fullPath.c_str());
            return;
        }
    ...
    AZ::Data::Asset<EditorMaterialAsset> EditorMaterialAssetBuilder::LoadEditorMaterialAsset(const AZStd::string& assetFullPath) const
    {
        ...
        AZ::Data::Asset<EditorMaterialAsset> Box3DEditorMaterialAsset;
        Box3DEditorMaterialAsset.Create(AZ::Data::AssetId(AZ::Uuid::CreateRandom()));

        auto* Box3DEditorMaterialAssetHandler = AZ::Data::AssetManager::Instance().GetHandler(EditorMaterialAsset::RTTI_Type());
        if (!Box3DEditorMaterialAssetHandler)
        {
            AZ_Error("EditorMaterialAssetBuilder", false, "Unable to find Box3D EditorMaterialAsset handler.");
            return {};
        }

        if (Box3DEditorMaterialAssetHandler->LoadAssetDataFromStream(Box3DEditorMaterialAsset, assetDataStream, nullptr) != AZ::Data::AssetHandler::LoadResult::LoadComplete)
        {
            AZ_Error("EditorMaterialAssetBuilder", false, "Failed to load Box3D EditorMaterialAsset: '%s'", assetFullPath.c_str());
            return {};
        }

        if (!Box3DEditorMaterialAsset)
        {
            AZ_Error("EditorMaterialAssetBuilder", false, "Box3D EditorMaterialAsset loaded with invalid data: '%s'", assetFullPath.c_str());
            return {};
        }

        return Box3DEditorMaterialAsset;
    }
